...
output_notBinary = open(outputFilename.replace('.','_justCurrentText.'), "w")
output_median_notBinary = open(outputFilename.replace('.','_currentMedian.'), "w")
#1. The hash
hasher = hashlib.md5()
...
#	print current;
	output_notBinary.write(str(current)+"\n")
        #Track the average
	index=qu.get()
	qu.put(index)
	...
	output_median_notBinary.write(str(np.median(window))+"\n")
        count += 1
	if (index== 50000.0 or index==90000.0):
		print np.median(window)
